...
class Task(models.Model):
    ...
    amount_total_min = models.PositiveIntegerField(verbose_name='每日总金额下限(万元)',
                                                   blank=False, null=False, help_text='每日总金额下限(万元),建议值2000')
    amount_total_max = models.PositiveIntegerField(verbose_name='每日总金额上限(万元)',
                                                   blank=False, null=False, help_text='每日总金额上限(万元),建议值4000')
    batch_total = models.PositiveSmallIntegerField(verbose_name='批次数', blank=False, null=False, 
                                                   help_text='每个任务生成的excel转账文件数,建议值15-30')
    batch_num_min = models.PositiveSmallIntegerField(verbose_name='每批次交易笔数下限',
                                                     blank=False, null=False, help_text='每批次交易笔数下限,建议值3')
    batch_num_max = models.PositiveSmallIntegerField(verbose_name='每批次交易笔数上限', 
                                                     blank=False, null=False, help_text='每批次交易笔数上限,建议值5')
